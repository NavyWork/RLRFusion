...
from plugin.RLRFusion.models.backbones.middle_fusion_layer import build_middle_fusion_layer
...
class Centerpoint_early_rcs_cat(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(Centerpoint_early_rcs_cat,
        ...
        if fusion_head is not None:
            modules = []
            for i in range(fusion_head.layers):
                modules.append(ConvModule(
                    in_channels=fusion_head.in_channels ,
                    out_channels=fusion_head.out_channels if i==(fusion_head.layers-1) else fusion_head.in_channels ,
                    kernel_size=3,
                    padding=1,
                    conv_cfg=dict(type='Conv2d'),
                    norm_cfg=dict(type='BN2d'),
                    bias='auto',))
            self.fusion_head = nn.Sequential(*modules)
        else:
            self.fusion_head = None

        if middle_fusion_layer:
    ...
    def extract_lidar_radar_feat_middle_fusion(self, lidar_pts_lst, radar_pts_lst):
        ...
        if self.use_LiDAR:
            ...
            lcoors, middle_feat_lidar = self.pts_middle_encoder(voxels_feat_lidar, coors_lidar, batch_size_lidar.item())
        else:
        ...
        if self.use_Radar:
            # Voxelize
            voxels_radar, num_points_radar, coors_radar = self.voxelize_radar(radar_pts_lst) # all point

            # Radar encoder
            #voxels_feat_radar, num_points_radar, coors_radar= self.voxel_fusion_layer(voxels_radar, num_points_radar, coors_radar)
            rcs, voxels_feat_radar, num_points_radar, coors_radar = self.pts_voxel_encoder_radar(voxels_radar, num_points_radar, coors_radar)
            # Middle encoder
            # middle_feat_lidar: (bs, C*D, H, W)
            batch_size_radar = coors_radar[-1, 0] + 1
            bev_feat_radar = self.pts_middle_encoder_radar(voxels_feat_radar, coors_radar, batch_size_radar.item())
        else:
            bakcbone_feat_radar = None

        middle_feat_lidar = self.middle_fusion_layer(middle_feat_lidar, voxels_feat_radar, coors_radar, batch_size_radar.item(), rcs)
        backbone_feat_lidar = self.pts_backbone(middle_feat_lidar)  # list

        # Go through backbone and fpn
        if self.with_pts_neck:
            x = self.pts_neck(backbone_feat_lidar)

        bev_pts = torch.cat([x[0], bev_feat_radar],dim=1)
        bev_pts = self.fusion_head(bev_pts)
        x[0] = bev_pts
    ...
    def extract_feat_v3(self, lidar_pts, img, img_metas, fused_points=None, radar=None, mode='train'):
        ...
        if self.use_Radar:
            ...
            if mode == 'train':
                radar_pts_lst = fused_points  # radar already is a list
            elif mode == 'test':
                radar_pts_lst = fused_points[0]  # changed the multi_apply so no need to squeeze the list
    ...
    def forward_train(self,
        ...
        img_feats, pts_feats, _ = self.extract_feat_v3(points, img=img, radar=radar, img_metas=img_metas,
